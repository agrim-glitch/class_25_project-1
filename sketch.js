...
function setup() {
	...
	packageSprite=createSprite(2300, 0, 10,10);
	packageSprite.addImage("hi",paper);
	packageSprite.scale =0.15

	groundSprite=createSprite(width/2, 660, 10000,10);


	bins = createSprite(2350,590,10,10);
    bins.addImage("binImg",bin);
	bins.scale =0.9
...
var options = {
	...
	packageBody = Bodies.rectangle(30,630 , 20 , 20,options);
...
function keyPressed() {
 if (keyCode === DOWN_ARROW) {
    ...
    Matter.Body.applyForce(packageBody,packageBody.position,{x:85,y:-85})
